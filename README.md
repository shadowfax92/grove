...
`Ctrl+Shift+Y` opens a centered maximize popup using `max_width` (default `60%` → 20% gap on each side, tuned for wide displays) and `max_height` (default `100%`). Set `max_width: "100%"` for a full-screen focus view. For normal panes, Grove swaps the pane with a temporary placeholder before opening the popup, then swaps it back on restore so the original layout slot is preserved.
